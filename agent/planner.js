...
export function buildPlannerMessages(objective, catalog, todayISO) {
  const system =
    ...
    "A step may use an earlier step's result: write {{t1.output.field}} (the earlier task id, then its output field) as the value, and list that task in depends_on.\n" +
...
export function executionLevels(ids, deps) {
  ...
  for (const i of ids) for (const d of deps[i] || []) { if (!out.has(d)) return null; /* a dependency on something that is not a task: no valid order exists */ indeg.set(i, indeg.get(i) + 1); out.get(d).push(i); }
...
export function validatePlan(raw, objective, catalog, opts = {}) {
  ...
  for (const t of tasks) {
    ...
    // what to remember: { memory_key: "output.path" }, so a result becomes project memory ONLY when the plan says so
    if (rawTask.remember && typeof rawTask.remember === "object" && !Array.isArray(rawTask.remember)) { const rem = {}; for (const [k, v] of Object.entries(rawTask.remember).slice(0, 5)) if (/^[a-z0-9_.-]{1,60}$/i.test(k) && typeof v === "string" && /^[A-Za-z0-9_.\[\]-]{1,120}$/.test(v)) rem[k] = v; if (Object.keys(rem).length) t.remember = rem; }
    // A tool's own schema decides how much a field may hold: most fields are short single lines, but code and data are long and keep their line breaks.
    if (rawIn && typeof rawIn === "object") { t.input = {}; for (const n of usable) { const o = rawIn[n], schema = (byName.get(n) || {}).input || {}; if (o && typeof o === "object" && !Array.isArray(o)) { const clean = {}; for (const [k, v] of Object.entries(o).slice(0, 8)) { const spec = schema[k] || {}, cap = spec.maxChars || 300; if (typeof v === "string") clean[str(k, 40)] = spec.multiline ? String(v).replace(/\r/g, "").slice(0, cap) : str(v, cap); else if (typeof v === "number" || typeof v === "boolean") clean[str(k, 40)] = v; else if (spec.type === "object" && v && typeof v === "object" && !Array.isArray(v)) { let js = ""; try { js = JSON.stringify(v); } catch (_) {} if (js && js.length <= cap) clean[str(k, 40)] = JSON.parse(js); } } if (Object.keys(clean).length) t.input[n] = clean; } } if (!Object.keys(t.input).length) delete t.input; }
