...
export class KnowledgeBase {
  ...
  // Rebuilds the base from chunks that were stored earlier (and their stored vectors, id -> Float32Array) WITHOUT embedding them again.
  // Only vectors the caller vouches for (made by this embedder) are passed in; chunks without one are still found by keyword.
  async load(chunks, vectors = new Map()) {
    this.chunks = chunks; this.byId = new Map(chunks.map((c) => [c.id, c])); this.bm25 = new BM25Index(chunks);
    this.vectorReady = false; this.vectorError = "";
    if (this.embedder && vectors.size) { if (!this.vectors) this.vectors = new MemoryVectorIndex(); await this.vectors.upsert([...vectors].map(([id, vector]) => ({ id, vector, meta: this.byId.get(id) ? this.byId.get(id).meta : {} }))); this.vectorReady = true; }
    return { chunks: chunks.length, vectors: this.vectorReady };
  }
...
export function verifyCitations(answer, passages) {
  ...
  for (const s of sentences) {
    ...
    const flat = plain.replace(/(\d),(?=\d{3})/g, "$1"), withUnit = flat.match(/\d+(?:\.\d+)?(?=\s*(?:%|per\s?cent))/g) || []; // a single digit still counts when it is a percentage
    const nums = [...new Set([...(flat.match(/\d+(?:\.\d+)?/g) || []).filter((n) => n.replace(".", "").length >= 2), ...withUnit])];
